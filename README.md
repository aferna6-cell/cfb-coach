...
**Why live capture failed + what to do (v1.9.7):** read [`docs/vision-capture-rca.md`](docs/vision-capture-rca.md). Short version: dxcam's "no frames" and the mss wrong crop were **our bugs** (fixed: DPI-aware ctypes window finder, cloaked-window filter, dxcam region clamp, errors shown in the heartbeat). The stream pausing when it loses focus is **Remote Play behavior** we can't fix. The 0.17 confidence meant **no grass/players found at all**. Diagnose on the laptop with `cfb-coach watch --probe --window "XBOX"`. Try `--capture wgc` (`pip install windows-capture`) for occlusion-proof window capture. Recommended production path: an **HDMI capture card with passthrough** → `watch --device 0`. Even then, shell/pressure from classical CV is not reliable: use vision for HUD + snap/play-end, and keep the defensive read typed.
